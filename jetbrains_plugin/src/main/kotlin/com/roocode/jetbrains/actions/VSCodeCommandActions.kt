...
fun executeCommand(commandId: String, project: Project?, vararg args: Any?, hasArgs: Boolean? = true) {
    ...
    try {
        val pluginContext = project.getService(com.roocode.jetbrains.core.PluginContext::class.java)
        ...
        val proxy = rpcProtocol.getProxy(com.roocode.jetbrains.core.ServiceProxyRegistry.ExtHostContext.ExtHostCommands)
        if (proxy == null) {
 ...
 */
class OpenDevToolsAction(private val getWebViewInstance: () -> com.roocode.jetbrains.webview.WebViewInstance?) :
